...
func Convert(hookFile string, h HAR, options lib.Options, minSleep, maxSleep uint, enableChecks bool, returnOnFailedCheck bool, batchTime uint, nobatch bool, correlate bool, only, skip []string) (result string, convertErr error) {
	...
	fprint(w, hookFile)
	fprint(w, "\n\n\n//================================================================================================\\\\\n")
	fprint(w, "//     *** You Shouldn't need to modify below this line.. It's autogenerated from the logs ***    \\\\")
	fprint(w, "\n//================================================================================================\\\\\n\n")
	fprint(w, "\n /** Generated by Kade's auto load tester */\n\n")
	fprintf(w, "// Version: %v\n", h.Log.Version)
	fprintf(w, "// Creator: %v\n", h.Log.Creator.Name)
	...
	// Temporary fix for https://github.com/luckybroman5/http-log-reconstructor/k6/issues/793
	if len(pages) == 0 {
		pages = []Page{{
	...
	for i, page := range pages {
		...
		if nobatch {
			...
			for entryIndex, e := range entries {
				...
				fprintf(w, "\t\tres = __applyKadeResponseHooks__(http.%s(", strings.ToLower(e.Request.Method))
			...
			for j, batchEntries := range batches {
				...
				fprint(w, "\t\treq = __applyKadeRequestHooks__([")
				...
				fprint(w, "]);\n")
				fprint(w, "\t\tres = __applyKadeResponseHooks__(http.batch(req));\n")
